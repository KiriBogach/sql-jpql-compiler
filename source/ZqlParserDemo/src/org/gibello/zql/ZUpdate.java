...
public class ZUpdate implements ZStatement {
	...
	/**
	 * Add one column=value pair to the SET... clause This method also keeps track
	 * of the column order
	 * 
	 ...
	 *          and values are ZExp objects (the column values). For example, the
	 *          values may be ZConstant objects (like "Smith") or more complex SQL
	 *          Expressions.
	 */
	 ...
	 * WARNING: This method will work only if column/value pairs have been inserted
	 * using addColumnUpdate() - otherwise it is not possible to guess what the
	 * right order is, and null will be returned.
	 * 
	 * @param num The column index (starting from 1).
	 * @return a ZExp, like a ZConstant representing a value, or a more complex SQL
	 *         expression.
	 */
	 ...
	 * 
	 * @return A Hashtable, where keys are column names (the columns to update), and
	 *         values are ZExp objects (Expressions that specify column values: for
