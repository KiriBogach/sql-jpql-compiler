...
public class ZInsert implements ZStatement {
	 ...
	 * 
	 * @param e An SQL expression or a SELECT statement. If it is a list of SQL
	 *          expressions, e should be represented by ONE SQL expression with
	 *          operator = "," and operands = the expressions in the list. If it is
	 *          a SELECT statement, e should be a ZQuery object.
	 */
	 ...
	 * 
	 * @return A ZQuery object (A SELECT statement), or null if there's no sub-query
	 *         (in that case, use the getValues() method to get the VALUES part).
	 */
	 ...
	 * 
	 * @return A vector of SQL Expressions (ZExp objects); If there's no VALUES but
	 *         a subquery, returns null (use getQuery() method).
	 */
